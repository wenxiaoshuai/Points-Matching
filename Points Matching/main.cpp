...
int main()
{
	...
	Size size(640, 540);//the dst image size
	resize(image01, image01, size);//resize image
	resize(image02, image02, size);

	//cv::SiftFeatureDetector detector(0, 5, 0.1, 5, 1.6);
	cv::SurfFeatureDetector detector(6000);
	std::vector<cv::KeyPoint> keyPoint1;
	std::vector<cv::KeyPoint> keyPoint2;
	detector.detect(image01, keyPoint1);
	detector.detect(image02, keyPoint2);
	drawKeypoints(image01, keyPoint1, image01, Scalar::all(-1), DrawMatchesFlags::DEFAULT);
	drawKeypoints(image02, keyPoint2, image02, Scalar::all(-1), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
	cv::imshow("KeyPoints of image1_SIFT", image01);
	imwrite("image1_SURF.jpg", image01);
	imwrite("image2_SURF.jpg", image02);
	cv::imshow("KeyPoints of image2_SIFT", image02);
	Mat imageDesc1, imageDesc2;
	detector.compute(image01, keyPoint1, imageDesc1);
	detector.compute(image02, keyPoint2, imageDesc2);
	...
	imwrite("SURF_Match.jpg", imageOutput);
